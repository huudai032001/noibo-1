import { shallowRef, markRaw, watch, ref, onMounted } from 'vue'
import type { ShallowRef, Ref } from 'vue'
import { MONTHS } from '../constants'
import type { ChartConfig } from '../models/human-resource.model'
import { zingChartTheme } from './zingchart-theme'
...
export function useHumanResourceChart(
  ...
  // Update chart data when chartData changes (from fetched API response)
  watch(
    () => chartData.value,
    (newChartData) => {
      chartDatatest.value = {
        ...
        series: [
          {
            type: 'bar',
            values: newChartData.startInYear,
            'background-color': '#472f92',
            'thousands-separator': '.',
            text: 'Tuyển thêm',
            'data-percent': 'nhân sự',
          ...
          {
            type: 'bar',
            values: newChartData.totalInYear,
            'background-color': '#f39c12',
            'thousands-separator': '.',
            text: 'Tổng',
            'data-percent': 'nhân sự',
          ...
          {
            type: 'line',
            values: newChartData.percentInYear,
            lineColor: '#e53935',
            marker: {
              backgroundColor: '#e53935',
            },
